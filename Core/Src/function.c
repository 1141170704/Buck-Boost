...
void OLED_Display(void)
{
    if (FlagSet.SMFlag != Err) // 非错误状态时
    {
        ...
        if (Screen_page == VIset_page) // 电压电流设置页面
        {
            ...
            if ((FlagSet.SMFlag == Rise) || (FlagSet.SMFlag == Run))                                          // 当输出开启时
            {
    ...
    else
    {
        ...
        if (getRegBits(FlagSet.ErrFlag, F_SW_IOUT_OCP)) // 判断是否输出过流保护状态
        {
            OLED_ShowChinese(32, 16, "输出过流");
        }
        if (getRegBits(FlagSet.ErrFlag, F_SW_SHORT)) // 判断是否短路保护状态
        {
            OLED_ShowChinese(32, 32, "输出短路");
        }
        if (getRegBits(FlagSet.ErrFlag, F_OTP)) // 判断是否短路保护状态
        {
...
void ShortOff(void)
{
    ...
    if ((Iout > MAX_SHORT_I) && (Vout < MIN_SHORT_V))
    {
        ...
        HAL_HRTIM_WaveformOutputStart(&hhrtim1, HRTIM_OUTPUT_TF1 | HRTIM_OUTPUT_TF2); // 开启HRTIM的PWM输出
        // 故障标志位
        setRegBits(FlagSet.ErrFlag, F_SW_SHORT);
        // 跳转至故障状态
        FlagSet.SMFlag = Err;
    }
    // 输出短路保护恢复
    ...
    if (getRegBits(FlagSet.ErrFlag, F_SW_SHORT))
    {
        ...
        if (RSCnt > 400)
        {
            ...
            else
            {
                ...
                RSNum++;
                // 清除过流保护故障标志位
                clearRegBits(FlagSet.ErrFlag, F_SW_SHORT);
            }
        }
    }
}
...
void OVP(void)
{
    ...
    if (Vout >= MAX_VOUT_OVP_VAL)
    {
        ...
        if (OVPCnt > 2)
        {
            ...
            HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TF1 | HRTIM_OUTPUT_TF2); // 关闭BOOST电路的PWM输出
            // 故障标志位
            setRegBits(FlagSet.ErrFlag, F_SW_VOUT_OVP);
            // 跳转至故障状态
            FlagSet.SMFlag = Err;
        }
    }
...
void OCP(void)
{
    ...
    if ((Iout >= MAX_VOUT_OCP_VAL) && (FlagSet.SMFlag == Run))
    {
        ...
        if (OCPCnt > 10)
        {
            ...
            HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TF1 | HRTIM_OUTPUT_TF2); // 关闭BOOST电路的PWM输出
            // 故障标志位
            setRegBits(FlagSet.ErrFlag, F_SW_IOUT_OCP);
            // 跳转至故障状态
            FlagSet.SMFlag = Err;
        }
    }
...
void OTP(void)
{
    ...
    if (TEMP >= MAX_OTP_VAL)
    {
        ...
        HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TD1 | HRTIM_OUTPUT_TD2); // 关闭BUCK电路的PWM输出
        HAL_HRTIM_WaveformOutputStop(&hhrtim1, HRTIM_OUTPUT_TF1 | HRTIM_OUTPUT_TF2); // 关闭BOOST电路的PWM输出
        setRegBits(FlagSet.ErrFlag, F_OTP);                                          // 故障标志位
        FlagSet.SMFlag = Err;                                                        // 跳转至故障状态
    }
}
...
CCMRAM void BBMode(void)
{
    ...
    if (PreBBFlag == FlagSet.BBFlag)
        FlagSet.BBModeChange = 0;
    else
        FlagSet.BBModeChange = 1;
}
